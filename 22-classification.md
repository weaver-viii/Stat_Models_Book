...
and then use OLS, GLS, LAD, etc to estimate the coefficients. What's the problem with this approach? First of all, there's no guarentee that once we've fitted the model (estimated the coefficients), that the predicted response, 
...
the corresponding distirbutional result assumed by logistic regression is
...
First, assuming that the $y\_i \| x\_i$'s are independent (this assuption is extremely important for inference, but somewhat less important for prediction if we have a proper test set with which to evaluate the prediction accuracy), the likelihood can be split up into
